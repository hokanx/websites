...
description: >
  ...
  barber, restaurant, gym, florist, garage...) using the Higgsfield MCP for all
  visuals. The page is one continuous "film": the visitor's scroll plays an
...
6. Everything generated goes through the Higgsfield MCP: submit jobs, keep
   working, poll job status, download results into the project.
...
Deploy per the project's hosting. Report to the user: live URL, the concept
spine in one sentence, the list of generated brand assets they now own (film
clips, stills, icon set, monogram/logo card, OG card), and anything honestly
